	...
	while (1) { /* endless loop broken by token returns it will generate a warning */
		...
		*/
		...
		case '=':
			c = readerGetChar(sourceBuffer);
			if (c == '=') {
				currentToken.code = OPEQ_T;
				scData.scanHistogram[currentToken.code]++;
			}
			else {
				readerRetract(sourceBuffer);
				currentToken.code = EQAS_T;
				scData.scanHistogram[currentToken.code]++;
			}
			return currentToken;
		case '!':
		...
		case '<':
			currentToken.code = OPGT_T;
			scData.scanHistogram[currentToken.code]++;
			return currentToken;
		case '>':
			currentToken.code = OPLT_T;
			scData.scanHistogram[currentToken.code]++;
			return currentToken;
		...
		*/
		...
		default: // general case
			...
			lexemeBuffer = readerCreate((int)lexLength + 1, 0, MODE_FIXED);
			if (!lexemeBuffer) {
...
int nextState(int state, int8 c) {
	int col;
	int next;
	col = nextClass(c);
