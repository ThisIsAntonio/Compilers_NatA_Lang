...
int64 getScannerFilesize(string fname);
...
int mainScanner(int argc, string* argv) {
	...
	if (sourceBuffer == NATA_NULL) {
		printScannerError("%s%s", argv[1], ": Could not create source buffer");
		exit(EXIT_FAILURE);
	}

	/* Open source file */
	if ((fileHandler = fopen(argv[2], "r")) == NATA_NULL) {
	...
	if (stringLiteralTable == NATA_NULL) {
	...
	sourceBuffer = stringLiteralTable = NATA_NULL;
	printScannerData(scData);
	/* Ass2 evaluation only */
	if (argv[3] != NATA_NULL && *argv[3] == 'l')
